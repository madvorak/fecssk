...
Rule `R` is derivable in `F` iff [for all formulas `φ`, `F ∪ {R}`-under-turnstile `φ` iff  `F`-under-turnstile `φ`].
Rule `R` is admissible in `F` iff `F ∪ {R}` is still consistent.
Formula `φ` is expressible in a logic `L` iff
[∃ formula `ψ` of `L` such that ∀ interpretations `v` we have `⁅φ⁆ᵥ = ⁅ψ⁆ᵥ`].
...
The witness for validity is a proof.
Sound formal system gives a sound procedure for validity (but not necessarily complete).
Sound complete formal system gives a sound semi-complete procedure for validity (may not terminate on invalid inputs).
...
Conclude that `φ` is valid or `¬φ` is satisfiable.
...
Homework: `(φ → ψ) → (ψ → χ) → (φ → χ)` in Hilbert system
